  ...
  return (
    <div className="w-full">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* LADO ESQUERDO: Formulário de Cadastro (Amplo) */}
        <div className="lg:col-span-7 xl:col-span-7">
          <form
            id="product-form-top"
            onSubmit={handleSubmit}
            className={`rounded-[2rem] border transition-all duration-500 bg-[#0c0a14] p-6 sm:p-8 shadow-[0_10px_40px_rgba(0,0,0,0.6)] relative overflow-hidden ${
              editingProductId || form.name
                ? "border-purple-500 shadow-[0_0_40px_rgba(109,40,217,0.25)] ring-1 ring-purple-500/30" 
            ...
            <div className="relative">
              ...
              <div className="mt-6 grid gap-5 md:grid-cols-2">
                <label className="grid gap-2 md:col-span-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Nome Comercial</span>
                  <input
                    required
                    value={form.name}
                    onChange={(event) => updateField("name", event.target.value)}
                    className="h-12 rounded-2xl border border-purple-500/25 bg-[#161324] px-4 text-sm text-white outline-none transition focus:border-purple-500 focus:bg-[#1c182e] focus:ring-2 focus:ring-purple-500/20 placeholder:text-white/30"
                    placeholder="Ex: Luminária Inteligente RGB"
                  />
                </label>

                <label className="grid gap-2 md:col-span-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Headline (Curta)</span>
                  <input
                    required
                ...
                <label className="grid gap-2 md:col-span-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Descrição Detalhada</span>
                  <textarea
                    required
                    ...
                    rows={4}
                    maxLength={3000}
                    className="rounded-2xl border border-purple-500/25 bg-[#161324] px-4 py-3 text-sm text-white outline-none transition focus:border-purple-500 focus:bg-[#1c182e] focus:ring-2 focus:ring-purple-500/20 placeholder:text-white/30"
                    placeholder="Descreva os principais benefícios... (Até 3000 caracteres)"
                  />
                </label>

                <label className="grid gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Preço Original / Sem Desconto (Opcional)</span>
                  <input
                    type="text"
                    inputMode="decimal"
                    value={form.oldPrice}
                ...
                <label className="grid gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Preço Atual / Real (R$)</span>
                  <input
                    required
                    type="text"
                    inputMode="decimal"
              ...
              <div className="mt-5 grid gap-5 sm:grid-cols-3">
                <label className="grid gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Avaliação (0 a 5.0)</span>
                  <input
                    type="text"
                    inputMode="decimal"
                    value={form.rating}
                ...
                <label className="grid gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Qtd Avaliações</span>
                  <input
                    type="number"
                    min="0"
                ...
                <label className="grid gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Vendas</span>
                  <input
              ...
              <div className="mt-5 grid gap-5 sm:grid-cols-2">
                <label className="grid gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Categoria</span>
                  <select
                    value={form.categorySlug}
                    onChange={(event) => updateField("categorySlug", event.target.value)}
                    className="h-12 rounded-2xl border border-purple-500/25 bg-[#161324] px-4 text-sm text-white outline-none transition focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20"
                  >
                    {categoryOptions.map((option) => (
                      <option key={option.value} value={option.value} className="bg-[#161324] text-white">
                        {option.label}
                      </option>
                    ))}
                  </select>
                ...
                <label className="grid gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Loja / Marketplace</span>
                  <select
                    value={form.iconKey}
                ...
                <label className="grid gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Texto do Selo</span>
                  <input
                    value={form.badge}
                ...
                <label className="grid gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Porcentagem OFF (Opcional)</span>
                  <input
                ...
                <label className="grid gap-2 md:col-span-2">
                  <div className="flex items-center justify-between ml-1">
                    <span className="text-xs font-bold uppercase tracking-widest text-white/90">Link de Afiliado (Shopee, ML ou Amazon)</span>
                    <button
                      type="button"
                  ...
                  <input
                    ...
                    className="h-12 rounded-2xl border border-purple-500/25 bg-[#161324] px-4 text-sm text-purple-300 outline-none transition focus:border-purple-500 focus:bg-[#1c182e] focus:ring-2 focus:ring-purple-500/20 font-medium placeholder:text-white/30"
                    placeholder="https://shope.ee/..."
                  />
                </label>

                <div className="grid gap-2 md:col-span-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-white/90 ml-1">Mídia do Produto</span>
                  <div className="flex gap-4 items-start">
                    <label className="flex flex-1 cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-purple-500/30 bg-[#161324] py-8 transition hover:bg-purple-950/30 hover:border-purple-500/50 group/upload text-center">
                    ...
                    {form.imageUrl && (
                      <div className="relative h-32 w-32 shrink-0 overflow-hidden rounded-2xl border border-purple-500/30 group/preview">
        ...
        {/* LADO DIREITO: Configurações, Colaboradores e Lista de Produtos */}
        <div className="lg:col-span-5 xl:col-span-5 space-y-8">
          <StoreSettingsManager />
          <CollaboratorManager />

          {/* Guia de Redação & Vitrine */}
          <div className="rounded-[2rem] border border-purple-500/20 bg-[#0c0a14] p-6 md:p-8 shadow-[0_10px_40px_rgba(0,0,0,0.6)] relative overflow-hidden">
            ...
            <div className="mt-6 space-y-4">
              <div className="flex gap-4 p-4 rounded-2xl bg-[#161324] border border-purple-500/20">
            ...
            <div className="mt-8 pt-6 border-t border-white/10">
              ...
              <Link
                href="/"
                className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-purple-500/30 bg-purple-950/40 p-4 text-xs font-black uppercase tracking-widest text-white transition hover:bg-purple-900/60 hover:border-purple-500/50"
              >
          ...
          <div className="space-y-4 max-h-[700px] overflow-y-auto pr-2 custom-scrollbar">
            <h3 className="text-sm font-black uppercase tracking-wider text-white/80 px-1">
              Gerenciar Produtos Cadastrados ({allProducts.length})
            </h3>
            {allProducts.length === 0 ? (
              <div className="rounded-[2rem] border border-dashed border-white/10 p-12 text-center bg-[#0c0a14]">
              ...
              allProducts.map((product) => (
                <div
                  ...
                  className={`group rounded-[2rem] border transition-all p-4 ${
                    product.isCustom ? 'bg-[#141024] border-purple-500/30' : 'bg-[#0c0a14] border-purple-500/15 opacity-80 hover:opacity-100'
                  }`}
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10 text-white transition hover:bg-purple-600 hover:text-white group-hover:scale-105"
                      ...
                      {product.isCustom && (
                        <button
                          ...
                          className="flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/10 text-red-400 transition hover:bg-red-600 hover:text-white group-hover:scale-105"
